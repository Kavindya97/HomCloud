    ...
    for file in fileList:
        ...
        for line in fin.readlines():
            if line [0] == "#" : # life death line
                ...
                if word in docwordList:
                    docweightList[docwordList.index(word)] += pow(weight, weight_exponent)
                else:
        ...
        for word in docwordList:
            if word not in corpuswordList:
                ...
                dwmatrix = np.append(dwmatrix, np.zeros((dwmatrix.shape[0],1)), axis = 1)
        # append new row 
        dwmatrix = np.append(dwmatrix, np.zeros((1,dwmatrix.shape[1])), axis = 0) 
        for word in docwordList:
...
def addArguments(parser):
    ...
    parser.add_argument('-e', '--exponent', type = float, nargs ='?', default = 1.0, help = 'exponent for weight. default = 1.0')
    parser.add_argument('-w', '--weight', type = str, nargs = '?', default = 'lt', help = 'variables as weight. b, d, or lt') 
    return parser


def main():
    parser=argparse.ArgumentParser(description = 'create topological feature vector table from cycle file')
    parser=addArguments(parser)
    args = parser.parse_args()
    doc_word_matrix, corpus_word_list = docs2dwmatrix(args.file_list, args.weight, args.exponent)
